 ...
 * 快速排序(优化版)
 */
public class QuickXSort implements ISort {
    @Override
    ...
    private Comparable[] sort(Comparable[] unsorted, int lo, int hi) {
        ...
        if ((hi - lo + 1) < INSERTION_SORT_CUTOFF) {
            return (new InsertSort()).sort(unsorted, lo, hi);
        }
    ...
    private int median3(Comparable[] unsorted, int lo, int hi) {
        int m = lo + (hi - lo + 1) / 2;

        int min = less(unsorted[lo], unsorted[m]) ? lo : m;
        min = less(min, unsorted[hi]) ? min : hi;
        return min;
    }
}
